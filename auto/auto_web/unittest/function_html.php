...
function getInputHeader($classKey, $class, $header, $headerName)
{
    ...
    foreach ($header as $key2 => $val2)
    {
        ...
        $html .= '<td>';
        if (strlen($val2['v']) < 60) {
            $html .= '<input type="text" name="' . $key2 . '" class="' . $key2 . ' ' .$classKey .'" placeholder="' . $val2['help'] . '" value="' . $val2['v'] .  '">';
        } else {
            $html .= '<textarea name="' . $key2 . '" class="' . $key2 . ' ' .$classKey  . ' auto-height" rows="6" cols="">'. $val2['v'] .'</textarea>';
        }
        $html .= '</td></tr>';
    }
    echo $html;
